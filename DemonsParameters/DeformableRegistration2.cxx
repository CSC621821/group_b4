...
//
  ...
  public:
    ...
    int iterations; // standard value: 50
...
int main( int argc, char *argv[] )
{
  if( argc < 8 )
    ...
    std::cerr << " [outputDisplacementFieldFile] numberOfHistogramLevels" << std::endl;
    std::cerr << " numberOfMatchPoints numberOfIterations standardDeviation" << std::endl;
    return EXIT_FAILURE;
    }
  ...
  setIterations(atoi(argv[argc-2]));
  setStandardDeviation(atof(argv[argc-1]));
  ...
  // Software Guide : BeginCodeSnippet
  filter->SetNumberOfIterations( getIterations() );
  ...
  if(argc > 8) // if a fourth line argument has been provided...
    {
  ...
  if(argc > 9) // if a fifth line argument has been provided...
    {
